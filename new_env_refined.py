...
class Quadruped_Env(gym.Env):
    ...
    def _compute_reward(self, torques, action, prev_action, prev_prev) -> float:
        ...
        r_forward = 3.0 * np.exp(- 4.0*(forward_vel - target_velo)**2) #changed from 2.5 , -5.0 -> 5.0, -10.0 to make the guassian distribution sharper(increased the weight too)
        ...
        r_height = -1.0 if z_height < 0.35 else 0.0
        ...
        total = r_forward+ r_yaw + r_alive + r_no_contact + r_energy + r_height + r_orient  + r_z_vel + r_slip + r_smooth + r_clear + r_lat
