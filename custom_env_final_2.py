...
class Quadruped_Env(gym.Env):
    ...
    def _contact_state(self, fid, idx, update) -> bool: #return true if in contact or false
        force = self.data.cfrc_ext[fid][2]
    ...
    def reset(self, seed=None, options=None):
        ...
        #Random values for Kp in +- 5.0 for dynamic randomization
        self.Kp_leg  = np.random.uniform(20, 30)
        self.Kp_hip = np.random.uniform(20, 30)

        #Random values for Kd in += 0.05 for dyanamic randomization
        
        self.Kd_leg = np.random.uniform(0.5, 0.75)
        self.Kd_hip = np.random.uniform(0.5, 0.75)
        ...
        target_vx = np.random.uniform(0.2, 0.8) 
    ...
    def _compute_reward(self, torques, action, prev_action, prev_prev):
        ...
        r_forward = 10 * np.exp(-25.0 * (forward_vel - target_velo)**2) #changed from 2.5 , -5.0
        ...
        r_height = -30 * np.square(z_height - 0.32) #changed from -50 -> -30 for smoother learning at the beginning
        ...
        #Alive reward
        r_alive = 0.1  #Reward scaling(new)

        #Penalizing yaw rate(to prevent the bot from moving in circles)
        yaw = self.data.qvel[5]
        r_yaw = - 0.2 * np.square(yaw)

        total = r_forward + r_energy + r_height + r_orient + r_alive + r_lat + r_pose + r_slip + r_smooth + r_yaw + r_clear
