...
class Quadruped_Env(gym.Env):
    ...
    def __init__(self, render_mode=None, xml_path=None):
        ...
        self.target_clearance_height = 0.07
        ...
        self.q_range_leg = np.array([0.4, 0.4, 0.65])
    ...
    def _contact_state(self, fid, idx, update) -> bool: #return true if in contact or false
        force = max(self.data.cfrc_ext[fid][2], 0)
    ...
    def reset(self, seed=None, options=None):
        ...
        body_mass_mult = np.random.uniform(0.95, 1.05)
        self.model.body_mass[self.base_body_id] = self.default_body_mass * body_mass_mult 

        '''
        #Random values for Kp in +- 0.6 for dynamic randomization
        self.Kp_leg  = np.random.uniform(19.4, 20.6)
        self.Kp_hip = np.random.uniform(34.4, 35.6)

        #Random values for Kd in += 0.05 for dyanamic randomization
        
        self.Kd_leg = np.random.uniform(0.15, 0.25)
        self.Kd_hip = np.random.uniform(0.30, 0.4)
        ...
        target_vx = np.random.uniform(0.4, 0.6) 
    ...
    def _compute_reward(self, torques, action, prev_action, prev_prev):
        ...
        r_forward = 5 * np.exp(-5.0 * (forward_vel - target_velo)**2) #changed from 2.5 , -5.0

        #Moving reward
        r_smooth = -0.05 * np.sum(np.square(action - prev_action)) -0.05 * (np.sum(np.square(action - 2*prev_action + prev_prev)))

        #HEIGHT PENALTY
        z_height = self.data.qpos[2]
        r_height = -50 * np.square(z_height - 0.35) #changed from -50 -> -30 for smoother learning at the beginning
        
        #Energy Penalty
        r_energy = -0.0015 * np.sum(np.square(torques)) #weight changed from 0.0015
        ...
        for contact, vel in ((c_fl, v_fl), (c_fr, v_fr), (c_rl, v_rl), (c_rr, v_rr)):
            if contact:
                ...
                r_slip -= 1.2 * slip_speed / max(abs(forward_vel), 0.25)
        ...
        r_clear = - 0.5 * clearance_error 
        '''

        # Stabilization
        rot_flat = self.data.xmat[self.base_body_id] 
        ...
        #Joint Pose penalty
        q = self.data.qpos[7:19]
        r_pose = - 0.5 * np.sum((q - self.q_default)**2) #changed from 0.5 -> 0.3 -> 0.1

        #LATERAL DRIFT penalty(to stop it from walking sideways)
        r_lat = - 0.5 * np.square(body_vel[1])
        ...
        r_yaw = - 0.5 * np.square(yaw)

        total = r_forward + r_energy + r_height + r_orient + r_alive + r_lat + r_pose + r_slip + r_smooth + r_yaw + r_z_vel
    ...
    def _is_fallen(self) -> bool:
        ...
        if abs(roll) > np.deg2rad(29) or abs(pitch) > np.deg2rad(34) or z_height < 0.12:
            terminated = True
        if self.step_count > 75 and self.last_forward_vel < 0.2:
            terminated = True
        return terminated
