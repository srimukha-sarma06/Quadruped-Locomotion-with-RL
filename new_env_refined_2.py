...
class Quadruped_Env(gym.Env):
    ...
    def _get_curriculum_param(self):
        if self.total_timesteps < 500000:
            return 0.0
        elif self.total_timesteps < 1000000:
            return 0.2
        elif self.total_timesteps < 1500000:
            return 0.4
        elif self.total_timesteps < 2000000:
            return 0.6
        elif self.total_timesteps < 2500000:
            return 0.8
        else:
            return 1.0
    ...
    def _compute_reward(self, torques, action, prev_action, prev_prev):
        ...
        r_forward = 2.5 * np.exp(- 5.0*(forward_vel - target_velo)**2) #changed from 2.5 , -5.0 -> 5.0, -10.0 to make the guassian distribution sharper(increased the weight too)
        ...
        r_ang_vel = 1.6 * np.exp(-5.0 * error)
        ...
        #Energy Penalty
        r_energy = - 0.0003 * np.sum(np.square(torques)) #weight changed from 0.0015 -> 0.0002
        ...
        for contact, vel in ((c_fl, v_fl), (c_fr, v_fr), (c_rl, v_rl), (c_rr, v_rr)):
            if contact:
                ...
                r_slip -= 0.5 * slip_speed / max(abs(forward_vel), 0.25)
        ...
        r_z_vel = - 0.3 * np.square(z_body_vel)
